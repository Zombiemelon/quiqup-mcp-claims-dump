/**
 * `set_collection_failed_batch` — staging-only QA helper.
 *
 * Marks one or more Last-Mile orders as collection-failed via
 * PUT /courier/orders/set_collection_failed. Lifted from the
 ...
 *
 * Failure reason: the upstream expects BOTH `failure_reason_uid`
 * (machine-readable, e.g. `future_delivery_request`) and `failure_reason`
 * (human-readable, e.g. "Future Delivery request"). Both are required —
 * the Postman example sends both even though they're semantically
 * redundant, so the contract requires it.
 */
...
export const spec: ToolSpec<typeof inputSchema, typeof outputSchema> = {
  name: "set_collection_failed_batch",
  description:
    "STAGING-ONLY. Mark one or more Last-Mile orders as `collection_failed` " +
    "(PUT /courier/orders/set_collection_failed, up to 10 orders per call). " +
  ...
  handler: async (auth, args) => {
    if (!auth.userId) {
      throw new Error(
        "set_collection_failed_batch requires an authenticated user",
      );
    }
    const jwt = await getQuiqupReadyJwt(auth.userId);
    const client = new QuiqupLastmileClient({ jwt, environment: "staging" });
    const data = await client.request("PUT", "/courier/orders/set_collection_failed", {
      body: {
      ...
      content: [
        {
          type: "text" as const,
          text:
            `Marked ${args.order_ids.length} order(s) collection_failed on staging ` +
