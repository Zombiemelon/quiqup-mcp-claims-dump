...
const STAGING_URL =
  "https://api.staging.quiqup.com/orders/batch/set_delivery_failed";

const VALID_ARGS = {
  order_ids: [111],
  failure_reason_uid: "customer_not_available",
  ...
  server.use(
    http.put(STAGING_URL, async ({ request }) => {
      ...
      return HttpResponse.json(
        opts.body ?? { orders: [{ id: 111, state: "delivery_failed" }] },
        { status },
      );
    }),
  );
...
describe("set_delivery_failed_batch", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    resetIdempotency();
    resetRateLimit();
  ...
  describe("registration", () => {
    it("exposes a spec with the expected name and STAGING-ONLY description", async () => {
      const mod = await import("../lib/tools/set-delivery-failed-batch");
      expect(mod.spec).toBeDefined();
      expect(mod.spec.name).toBe("set_delivery_failed_batch");
      expect(mod.spec.description).toMatch(/STAGING-ONLY/);
      expect(mod.spec.description).toMatch(/delivery_failed/);
    });
  ...
  describe("happy path", () => {
    it("PUTs https://api.staging.quiqup.com/orders/batch/set_delivery_failed with ids + failure fields", async () => {
      ...
      expect(block.text).toMatch(/customer_not_available/);
    });
  });
  ...
  describe("upstream 422 via the registerTool wrapper", () => {
    it("returns isError:true with the upstream body (no exception leaks)", async () => {
      stubStagingPut({
        status: 422,
        body: { error: "order not out for delivery" },
      });
      const mod = await import("../lib/tools/set-delivery-failed-batch");
      ...
      expect(block.text).toContain("order not out for delivery");
    });
  });
  ...
  describe("idempotency", () => {
    it("returns the cached result on a second call with the same key + args", async () => {
      ...
      const args = { ...VALID_ARGS, idempotency_key: "logical-df-1" };
