...
const STAGING_URL =
  "https://api.staging.quiqup.com/orders/batch/set_out_for_delivery";
  ...
  server.use(
    http.put(STAGING_URL, async ({ request }) => {
      ...
      return HttpResponse.json(
        opts.body ?? { orders: [{ id: 111, state: "out_for_delivery" }] },
        { status },
      );
    }),
  );
...
describe("set_out_for_delivery_batch", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    resetIdempotency();
    resetRateLimit();
  ...
  describe("registration", () => {
    it("exposes a spec with the expected name and STAGING-ONLY description", async () => {
      const mod = await import("../lib/tools/set-out-for-delivery-batch");
      expect(mod.spec).toBeDefined();
      expect(mod.spec.name).toBe("set_out_for_delivery_batch");
      expect(mod.spec.description).toMatch(/STAGING-ONLY/);
      expect(mod.spec.description).toMatch(/out_for_delivery/);
    });
  ...
  describe("happy path", () => {
    it("PUTs https://api.staging.quiqup.com/orders/batch/set_out_for_delivery with the supplied ids", async () => {
  ...
  describe("upstream 422 via the registerTool wrapper", () => {
    it("returns isError:true with the upstream body (no exception leaks)", async () => {
      stubStagingPut({
        status: 422,
        body: { error: "order not in pending state" },
      });
      const mod = await import("../lib/tools/set-out-for-delivery-batch");
      ...
      expect(block.text).toContain("order not in pending state");
    });
  });
  ...
  describe("idempotency", () => {
    it("returns the cached result on a second call with the same key + args", async () => {
      ...
      const args = { order_ids: [111], idempotency_key: "logical-ofd-1" };
